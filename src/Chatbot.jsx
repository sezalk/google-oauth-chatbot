...
const ChatMessage = ({ message }) => {
  ...
  return (
    <li className={`chat ${incoming ? 'incoming' : 'outgoing'}`}>
      <span className="material-symbols-outlined">{incoming ? '🤖' : ''}</span>
      <p>{text}</p>
    </li>
  );
};
  ...
  return (
    <form className="chat-input">
      <textarea
        ...
        required
      ></textarea>
      <span id="send-btn" className="material-symbols-rounded">▶️</span>
    </form>
  );
};
  ...
  return (
    <div>
      <div className="chatbot">
        <header>
          <h2>Chatbot</h2>
          <span className="close-btn material-symbols-outlined" onClick={toggleChatbot}>❌</span>
        </header>
      ...
      <button className="chatbot-toggler" onClick={toggleChatbot}><span className="material-symbols-rounded">🗨️</span>
